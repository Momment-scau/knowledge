...
- 概述
    ...
    1. 模块的加载步骤：
        ...
        2. 执行模块的源码：
            ...
            1. `require`第一次加载某模块，执行整个脚本，在内存生成一个缓存对象（`{id:'...',exports:{...},...}`）。
            3. 无论加载多少次，仅在第一次加载时运行，除非手动清除系统缓存。
            3. x模块执行时出现`require(y)`则进入y模块代码中执行，执行完毕后再回到x模块继续向下执行。
            4. 一旦出现某个模块被“循环加载”，则模块对象的`exports`仅保存已经执行的部分，还未执行的部分等到代码返回到此模块第一次被引用时执行。
        3. 将模块的`exports`值输出至缓存，以供其他模块`require`获取。
    2. 运行时进行模块加载。
    3. 每个文件就是一个模块，有自己单独作用域，不污染全局作用域。
    4. 模块加载的顺序，按照其在代码中出现的顺序、引用则嵌套加载。
    5. `require(x模块)`返回内存中x模块的`module.exports`指向的值，是**动态绑定**，可以重新赋值和属性改写（属性改写会改变x模块的缓存值，所有`require(x模块)`都会共享）。
